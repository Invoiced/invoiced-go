...
func TestUnMarshalInvoiceObject(t *testing.T) {
  ...
  "calculate_taxes": true
	...
	if so.Items[0].Id != 7 {
		t.Fatal("ItemClient 0 has incorrect id")
	}

	if so.Items[0].Type != "product" {
		t.Fatal("ItemClient 0 has incorrect type")
	}
	...
	if so.Items[0].Quantity != 1.0 {
		t.Fatal("ItemClient 0 has incorrect quantity")
	}

	if so.Items[0].UnitCost != 45 {
		t.Fatal("ItemClient 0 has incorrect unit cost")
	}

	if so.Items[0].Amount != 45 {
		t.Fatal("ItemClient 0 has incorrect amount")
	}

	if !so.Items[0].Taxable {
		t.Fatal("ItemClient 0 should be taxable")
	}
